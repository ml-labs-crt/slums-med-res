#In this version the classifier is loaded with 0s and 1s. The dataset was shuffled.
#library(raster)
setwd(".../LowResolution")
...
y_train80Class <- as.integer(X_train80[,11])
X_train80Scaled[,11] <- y_train80Class
names(X_train80Scaled)[11] <- "class"
y_test20Class <- as.integer(X_test20[,11])
...
results <- data.frame(y_test20Class+1,prediction)
...
ResultMedellin <- data.frame(accuracySlums,accuracyNonSlum,IoUSlum, IoUNonSlum, MeanIoU)
CCFMedellin <- end_time - start_time