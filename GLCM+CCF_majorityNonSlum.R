...
#Removing Inf, -Inf and NA values
sum(is.infinite(df_GLCM$glcm_correlation),is.infinite(df_GLCM$glcm_correlation.1),is.infinite(df_GLCM$glcm_correlation.2),is.infinite(df_GLCM$glcm_correlation.3))
df_GLCM$glcm_correlation.1[is.infinite(df_GLCM$glcm_correlation.1)] <- NA
df_GLCM$glcm_correlation.2[is.infinite(df_GLCM$glcm_correlation.2)] <- NA
df_GLCM$glcm_correlation.3[is.infinite(df_GLCM$glcm_correlation.3)] <- NA
library(tidyr)
...
library(dplyr)
set.seed(0)
BalancedNonSlums <- sample_n(nonslum_GLCM, nrow(slum_GLCM))
floor <- round(0.8*nrow(BalancedNonSlums))
set.seed(0)
rows <- sample(nrow(BalancedNonSlums),floor,replace = FALSE)
trainSlum <- slum_GLCM[rows,]
testSlum <- slum_GLCM[-rows,]
